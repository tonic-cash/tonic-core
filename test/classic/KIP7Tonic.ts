...
import { BadRecipient, ERC20Token, KIP7Token, KIP7Tonic } from '@/typechain-types'
...
describe('KIP7Tonic', () => {
  ...
  before(async () => {
    ...
    const tonicFactory = await ethers.getContractFactory('KIP7Tonic')
    ...
    badRecipient = (await (await ethers.getContractFactory('BadRecipient')).deploy()) as BadRecipient
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      const receipt = await tonic
        .connect(relayer)
        .withdraw(proof, ...args, { value: refund, from: relayer.address })
      const events = (await receipt.wait()).events || []
      ...
      expect(balanceTonicAfter).to.equal(
        BigNumber.from(balanceTonicBefore).sub(BigNumber.from(tokenDenomination)),
      )
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let error = await tonic.connect(relayer).withdraw(proof, ...args, { value: 1, from: relayer.address })
        .should.rejected
  ...
  describe('Token Transfers & KIP7', function () {
    ...
    it('reverts if requested token is deposit token', async function () {
      ...
      const error = await tonic['transfer(address,address,uint256)'](owner.address, token.address, amount)
        .should.rejected
    ...
    it('reverts if not called by owner', async function () {
      ...
      const error = await tonic
        .connect(stranger)
        ['transfer(address,address,uint256)'](stranger.address, airdropToken.address, amount).should.rejected
