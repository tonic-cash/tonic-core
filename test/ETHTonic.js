...
const Tonic = artifacts.require('ETHTonic')
...
contract('ETHTonic', (accounts) => {
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await tonic.deposit(commitment, { value, from: sender })
      ...
      ;({ logs } = await tonic.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await tonic.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await tonic.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      const gasCost = await tonic.deposit.estimateGas(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      await tonic.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })

      const balanceUserAfter = await web3.eth.getBalance(user)
      balanceUserAfter.should.be.lte.BN(toBN(balanceUserBefore).sub(toBN(value)).sub(toBN(gasCost)))
      ...
      const balanceTonicBefore = await web3.eth.getBalance(tonic.address)
      ...
      const { logs } = await tonic.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceTonicAfter = await web3.eth.getBalance(tonic.address)
      ...
      balanceTonicAfter.should.be.eq.BN(toBN(balanceTonicBefore).sub(toBN(value)))
      ...
      const expectedBalance = toBN(balanceReceiverBefore).add(toBN(value)).sub(feeBN);
      const percentage = 97;
      const expectedBalance97Percent = expectedBalance.muln(percentage).divn(100);
      ...
      const isWithinRange = toBN(balanceReceiverAfter).gte(lowerBound) && toBN(balanceReceiverAfter).lte(upperBound);
      ...
      logs[0].args.relayer.should.be.equal(operator)
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      tree.insert(deposit.commitment)
      await tonic.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await tonic.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await tonic.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await tonic.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await tonic.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await tonic.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await tonic.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await tonic.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await tonic.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await tonic.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
