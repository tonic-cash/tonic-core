...
import "../../Interfaces/IFuckingKIP7Receiver.sol";
...
contract KIP7 is Context, KIP13, IKIP7, IKIP7Metadata {
   ...
   * - the caller must have a balance of at least `amount`.
   * - if `recipient` is a smart contract, it must implement {IFuckingKIP7Receiver}
   */
   ...
   * forwarded in {IFuckingKIP7Receiver-onKIP7Received} to contract recipients.
   *
   * Emits a {Transfer} event
   */
   ...
   * - `to` cannot be the zero address.
   * - `from` must have a balance of at least `amount`.
   * - if `to` is a contract it must implement {IFuckingKIP7Receiver} and
   * - If `to` refers to a smart contract, it must implement {IFuckingKIP7Receiver}, which is called upon
   *   a safe transfer.
   */
  function _safeTransfer(address from, address to, uint256 amount, bytes memory _data) internal virtual {
    _transfer(from, to, amount);
    require(_checkOnKIP7Received(from, to, amount, _data), "KIP7: transfer to non IFuckingKIP7Receiver implementer");
  }
   ...
   * - If `account` refers to a smart contract, it must implement {IFuckingKIP7Receiver-onKIP7Received},
  ...
  function _safeMint(address account, uint256 amount, bytes memory _data) internal virtual {
    ...
    require(_checkOnKIP7Received(address(0), account, amount, _data), "KIP7: transfer to non IFuckingKIP7Receiver implementer");
  }
  ...
  /**
   * @dev Internal function to invoke {IFuckingKIP7Receiver-onKIP7Received} on a target address.
  ...
  function _checkOnKIP7Received(address from, address to, uint256 amount, bytes memory _data) private returns (bool) {
    if (to.isContract()) {
      try IFuckingKIP7Receiver(to).onKIP7Received(_msgSender(), from, amount, _data) returns (bytes4 retval) {
        return retval == IFuckingKIP7Receiver.onKIP7Received.selector;
